...
void SparkDrawable::addImage(const std::string& name, ParticleTexture *particleTexture, GLuint type, GLuint clamp)
{
	if (particleTexture && particleTexture->getImage().valid())
	{
		ImageAttribute attr;
		attr.particleTex = particleTexture;
		attr.type = type;
		attr.clamp = clamp;
	...
	{
		OSG_WARN << "SparkDrawable::ParticleTexture NULL" << std::endl;
	}
}
	...
	if (_dirty)
	{
		if (_baseSystemCreator)
		{
				...
				itr != _textureObjMap.end(); ++itr)
			{
				...
				ParticleTexture *particleTex = attr.particleTex;
				if (!particleTex->isCompiled())
				{
					int glTexId = compileInternalTexture(particleTex->getImage().get(), attr.type, attr.clamp);
					textureIDMap[itr->first] = glTexId;
					particleTex->setCompiledAndGLTextureId(glTexId);
				}
				else
				{
					textureIDMap[itr->first] = particleTex->getGLTextureId();
				}
			}

			// TODO: change the hardcoded 800x600
