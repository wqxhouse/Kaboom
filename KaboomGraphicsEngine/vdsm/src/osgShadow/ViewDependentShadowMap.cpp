    ...
    _textureUnit(0)
{
    ...
    else if (settings->getShadowMapTechniqueHints() & osgShadow::ShadowSettings::VARIANCE)
    {
        // VSM requires a RGB32 texture for casting shadows
    ...
    else
    {
        ...
        // attach the texture and use it as the color buffer.
        if (settings->getShadowMapTechniqueHints() & osgShadow::ShadowSettings::VARIANCE)
        {
            _camera->attach(osg::Camera::COLOR_BUFFER, _texture.get());
        }
        else
...
// Frustum
//
ViewDependentShadowMap::Frustum::Frustum(osgUtil::CullVisitor* cv, double minZNear, double maxZFar, double maxDistance):
    corners(8),
    faces(6),
    edges(12)
{
    ...
    if (cv->getComputeNearFarMode()!=osg::CullSettings::DO_NOT_COMPUTE_NEAR_FAR)
    {
        ...
        if (maxDistance < zFar-zNear) zFar = zNear + maxDistance;
    ...
    computeFrustumParameters();
}

void ViewDependentShadowMap::Frustum::computeFrustumParameters()
{
...
double ViewDependentShadowMap::Frustum::computeSplit(unsigned int index, unsigned int numMaps, double globalNear, double globalFar, bool useCustom, bool orthographic)
{
    double temp_near = globalNear, temp_far = globalFar;
    if (!useCustom)
    {
#if 0  // linear
        temp_near = globalNear + (globalFar - globalNear) * ((double)index / (double)numMaps);
        temp_far = globalNear + (globalFar - globalNear) * ((double)(index+1) / (double)numMaps);
#else  // exponential: Ci = (f - n) * (i/numsplits)^(bias+1) + n;
        static double splitSchemeBias[2]= {0.25, 0.66};
        std::vector<double> splitDistances(numMaps + 1);
        splitDistances[numMaps] = globalFar;
        for (unsigned int i = 0; i < numMaps; ++i) 
        {
            double IDM = (double)(i) / (double)(numMaps);
            splitDistances[i] = (globalFar - globalNear) * (pow(IDM, splitSchemeBias[1] + 1.0)) + globalNear;
        }
    
        temp_near = splitDistances[index];
        temp_far = splitDistances[index+1];
#endif
    }
    
    double splittedZ = (osg::Vec3d(0.0,0.0,-temp_far) * projectionMatrix).z();
    double l, r, b, t, zn, zf;
    if (orthographic)
    {
        projectionMatrix.getOrtho(l, r, b, t, zn, zf);
        projectionMatrix.makeOrtho(l, r, b, t, temp_near, temp_far);
    }
    else
    {
        projectionMatrix.getFrustum(l, r, b, t, zn, zf);
        double zFactor = temp_near / zn;
        if (zFactor > 0.0)
        {
            l = l * zFactor; r = r * zFactor;
            b = b * zFactor; t = t * zFactor;
        }
        projectionMatrix.makeFrustum(l, r, b, t, temp_near, temp_far);
    }
    return splittedZ;
}
...
void ViewDependentShadowMap::cull(osgUtil::CullVisitor& cv)
{
    ...
    Frustum frustum(&cv, minZNear, maxZFar, settings->getMaximumShadowMapNearFarDistance());
    double global_near = 0.0, global_far = 0.0;
    if (orthographicViewFrustum)
    {
        global_near = (frustum.projectionMatrix(3,2) + 1.0) / frustum.projectionMatrix(2,2);
        global_far = (frustum.projectionMatrix(3,2) - 1.0) / frustum.projectionMatrix(2,2);
    }
    else
    {
        global_near = frustum.projectionMatrix(3,2) / (frustum.projectionMatrix(2,2) - 1.0);
        global_far = frustum.projectionMatrix(3,2) / (1.0 + frustum.projectionMatrix(2,2));
    }
    ...
    if (numShadowMapsPerLight>1 && !(settings->getShadowMapTechniqueHints() & osgShadow::ShadowSettings::PARALLEL_SPLIT))
    {
        OSG_NOTICE<<"numShadowMapsPerLight of "<<numShadowMapsPerLight<<" can only be supported in parallel-split mode, falling back to 1."<<std::endl;
        numShadowMapsPerLight = 1;
        settings->setNumShadowMapsPerLight(numShadowMapsPerLight);
    }
        ...
        ++itr)
    {
        ...
        // For each light/shadow map
        Frustum* pFrustum = &frustum;
        for (unsigned int sm_i=0; sm_i<numShadowMapsPerLight; ++sm_i)
        {
            Frustum subFrustum;
            if (numShadowMapsPerLight > 1)
            {
                subFrustum.modelViewMatrix = frustum.modelViewMatrix;
                subFrustum.projectionMatrix = frustum.projectionMatrix;
                
                double splittedZ = 1.0;
                const std::vector<osg::Vec2d>& splitValues = settings->getSplitValues();
                if (sm_i < splitValues.size())
                {
                    double temp_near = osg::clampBetween(global_near + splitValues[sm_i][0], global_near, global_far);
                    double temp_far = osg::clampBetween(global_near + splitValues[sm_i][1], global_near, global_far);
                    if (temp_far <= temp_near) temp_far = global_far;
                    splittedZ = subFrustum.computeSplit(sm_i, numShadowMapsPerLight, temp_near, temp_far, true, orthographicViewFrustum);
                }
                else
                    splittedZ = subFrustum.computeSplit(sm_i, numShadowMapsPerLight, global_near, global_far, false, orthographicViewFrustum);
                if (sm_i < _splitUniforms.size()) _splitUniforms[sm_i]->set((float)splittedZ);
                
                subFrustum.computeFrustumParameters();
                pFrustum = &subFrustum;
            }

            // 3.1 compute light space polytope
            //
            osg::Polytope polytope = computeLightViewFrustumPolytope(*pFrustum, pl);

            // if polytope is empty then no rendering.
            if (polytope.empty())
            {
                OSG_NOTICE<<"Polytope empty no shadow to render"<<std::endl;
                ++textureUnit;  // still increase texunit count with regard to the shadow split ID
                continue;
            }
            ...
            if (!computeShadowCameraSettings(*pFrustum, pl, projectionMatrix, viewMatrix))
            {
            ...
            if (/*numShadowMapsPerLight>1 &&*/ _shadowedScene->getCastsShadowTraversalMask()!=0xffffffff)
            {
                ...
                if (clsb._bb.valid() && (clsb._bb.xMin()>-1.0f || clsb._bb.xMax()<1.0f || clsb._bb.yMin()>-1.0f || clsb._bb.yMax()<1.0f))
                {
...
#if 0  // deprecated method to split the light frustum directly
            double splitPoint = 0.0;
        
            if (numShadowMapsPerLight>1)
            {
                osg::Vec3d eye_v = pFrustum->eye * viewMatrix;
                osg::Vec3d center_v = pFrustum->center * viewMatrix;
...
#endif
            // Create shadow data and RTT camera
            osg::ref_ptr<ShadowData> sd;

            if (previous_sdl.empty())
            {
                OSG_INFO<<"Create new ShadowData"<<std::endl;
                sd = createShadowData(vdd);

                const ShadowSettings::ShaderGenerator* generator = settings->getShaderGenerator();
                if (generator)
                {
                    std::string casterVertexShader = generator->generateCasterVertexShader(settings);
                    std::string casterFragmentShader = generator->generateCasterFragmentShader(settings);

                    osg::ref_ptr<osg::Program> program = new osg::Program;
                    if (!casterVertexShader.empty()) program->addShader(new osg::Shader(osg::Shader::VERTEX, casterVertexShader));
                    if (!casterFragmentShader.empty()) program->addShader(new osg::Shader(osg::Shader::FRAGMENT, casterFragmentShader));
                    if (program->getNumShaders() > 0) sd->_camera->getOrCreateStateSet()->setAttributeAndModes(program.get());
                }
            }
            else
            {
                OSG_INFO<<"Taking ShadowData from of previous_sdl"<<std::endl;
            ...
            if (!orthographicViewFrustum && (settings->getShadowMapTechniqueHints() & osgShadow::ShadowSettings::PERSPECTIVE))
            {
                adjustPerspectiveShadowMapCameraSettings(vdsmCallback->getRenderStage(), *pFrustum, pl, camera.get());
            ...
            // 3.4 compute main scene graph TexGen + uniform settings + setup state
            //
            assignTexGenSettings(&cv, camera.get(), textureUnit, sd->_texgen.get());
            
            // create texture and frustum geometry data for debugging
            handleDebugData(sm_i, sd->_texture.get(), camera->getViewMatrix(), camera->getProjectionMatrix());
...
void ViewDependentShadowMap::createShaders()
{
    ...
    for(unsigned int sm_i=0; sm_i<settings->getNumShadowMapsPerLight(); ++sm_i)
    {
        ...
        if (settings->getShadowMapTechniqueHints() & osgShadow::ShadowSettings::PARALLEL_SPLIT)
        {
            std::stringstream sstr;
            sstr<<"shadowSplittedZ"<<sm_i;
            osg::ref_ptr<osg::Uniform> shadowSplittedZ = new osg::Uniform(sstr.str().c_str(), (float)1.0f);
            _splitUniforms.push_back(shadowSplittedZ.get());
        }
    }
    ...
    switch(settings->getShaderHint())
    {
        ...
        case(ShadowSettings::PROVIDE_VERTEX_AND_FRAGMENT_SHADER):
        {
            if (generator && !debug)
            {
                std::string vertexCode = generator->generateReceiverVertexShader(settings);
                std::string fragmentCode = generator->generateReceiverFragmentShader(settings);
                
                _program = new osg::Program;
                if (!vertexCode.empty()) _program->addShader(new osg::Shader(osg::Shader::VERTEX, vertexCode));
                if (!fragmentCode.empty()) _program->addShader(new osg::Shader(osg::Shader::FRAGMENT, fragmentCode));
            }
            break;
        }
...
bool ViewDependentShadowMap::computeShadowCameraSettings(Frustum& frustum, LightData& positionedLight, osg::Matrixd& projectionMatrix, osg::Matrixd& viewMatrix)
{
    ...
    if (positionedLight.directionalLight)
    {
        ...
        // Make the X-Y range a square to avoid texture coordinate errors
        double diff = ((xMax - xMin) - (yMax - yMin)) * 0.5;
        if (diff < 0.0)
        {
            xMin += diff;
            xMax -= diff;
...
osg::StateSet* ViewDependentShadowMap::selectStateSetForRenderingShadow(ViewDependentData& vdd) const
{
    ...
    for(Uniforms::const_iterator itr=_splitUniforms.begin();
        itr!=_splitUniforms.end();
        ++itr)
    {
        OSG_INFO<<"addUniform("<<(*itr)->getName()<<")"<<std::endl;
        stateset->addUniform(itr->get());
...
void ViewDependentShadowMap::handleDebugData(unsigned int sm_i, osg::Texture* texture, const osg::Matrix& viewMatrix, const osg::Matrix& projMatrix)
{
    if (_debugTextures.size() > 0)
    {
        if (sm_i < _debugTextures.size()) _debugTextures[sm_i] = texture;
        else _debugTextures.push_back(texture);
    }
    if (_debugGeode.valid())
    {
        osg::Geometry* geom = NULL;
        if (sm_i < _debugGeode->getNumDrawables())
            geom = _debugGeode->getDrawable(sm_i)->asGeometry();
        else
        {
            static osg::Vec4 inbuiltColors[4] = {
            ...
            (*colors)[0] = inbuiltColors[_debugGeode->getNumDrawables() % 4];

            osg::DrawElementsUByte* side = new osg::DrawElementsUByte(GL_QUAD_STRIP, 10);
            (*side)[0] = 0; (*side)[1] = 4; (*side)[2] = 1; (*side)[3] = 5;
            (*side)[4] = 2; (*side)[5] = 6; (*side)[6] = 3; (*side)[7] = 7;
            (*side)[8] = 0; (*side)[9] = 4;

            osg::DrawElementsUByte* top = new osg::DrawElementsUByte(GL_QUADS, 8);
            (*top)[0] = 0; (*top)[1] = 1; (*top)[2] = 2; (*top)[3] = 3;
            (*top)[4] = 4; (*top)[5] = 5; (*top)[6] = 6; (*top)[7] = 7;
            ...
            geom->addPrimitiveSet(top);
            _debugGeode->addDrawable(geom);
        }

        if (geom)
        {
            osg::Vec3Array* corners = dynamic_cast<osg::Vec3Array*>(geom->getVertexArray());
            if (corners && corners->size()==8)
            {
                (*corners)[0].set(-1.0f,-1.0f,-1.0f); (*corners)[1].set( 1.0f,-1.0f,-1.0f);
                (*corners)[2].set( 1.0f,-1.0f, 1.0f); (*corners)[3].set(-1.0f,-1.0f, 1.0f);
                (*corners)[4].set(-1.0f, 1.0f,-1.0f); (*corners)[5].set( 1.0f, 1.0f,-1.0f);
                (*corners)[6].set( 1.0f, 1.0f, 1.0f); (*corners)[7].set(-1.0f, 1.0f, 1.0f);

                osg::Matrixd clipToWorld;
                clipToWorld.invert(viewMatrix * projMatrix);
                for (int i = 0; i < 8; ++i)
