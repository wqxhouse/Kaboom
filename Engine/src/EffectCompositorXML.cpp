...
osg::Camera* EffectCompositor::createPassFromXML(osgDB::XmlNode* xmlNode)
{
	...
	for (unsigned int i = 0; i < xmlNode->children.size(); ++i)
	{
		...
		if (childName == "output_buffer")
		{
			...
			// TODO: Currently, camera buffer size is determined by the size of 
			// the last texture attached. The optimal size of the buffer should be 
			// the size of the largest texture.
		...
		else if (childName == "uniform_buffer")
		{
			if (xmlChild->children.size() > 0)
			{
				osg::UniformBufferBinding *ubb= createUniformBufferFromXML(xmlChild, false);
				stateset->setAttributeAndModes(ubb, osg::StateAttribute::ON);

				std::string uniformBufferName = xmlChild->getTrimmedContents();
			...
			{
				std::string uniformBufferName = xmlChild->getTrimmedContents();
				osg::UniformBufferBinding *ubb = getUniformBufferBinding(uniformBufferName);
				...
				{
					OSG_NOTICE << "EffectCompositor: <pass> can't find global uniform_buffer object " << xmlChild->getTrimmedContents() << std::endl;
				}
			}
		}
...
osg::Texture* EffectCompositor::createTextureFromXML(osgDB::XmlNode* xmlNode, bool asGlobal)
{
	...
	for (unsigned int i = 0; i < xmlNode->children.size(); ++i)
	{
		...
		else if (childName == "internal_format")
		{
			...
			// need to work with GL_RED_INTEGER_EXT source format on Intel HD4400
			else if (format == "red32i") texture->setInternalFormat(GL_R32I); 
			else if (format == "red16i") texture->setInternalFormat(GL_R16I);
		...
		else if (childName == "source_format")
		{
			...
			else if (format == "redi") texture->setSourceFormat(GL_RED_INTEGER_EXT);
		...
		else if (childName == "binding")
		{
			...
			int unit = atoi(bindStr.c_str());
			texture->bindToImageUnit(unit, osg::Texture::READ_WRITE);
		}
		else
...
// TODO: implement set array by element later
// Currently, default value loops the whole buffer
...
// TODO2: support double type
// TODO3: support uniform_buffer element animation

osg::UniformBufferBinding* EffectCompositor::createUniformBufferFromXML(osgDB::XmlNode *xmlNode, bool asGlobal)
{
	std::string name = xmlNode->properties["name"];
	if (name.empty())
		OSG_NOTICE << "EffectCompositor: inappropriate to create uniform buffer with empty name" << std::endl;
	
	std::string sizeStr = xmlNode->properties["size"];
	int size = 1;
	if (!sizeStr.empty())
	{
		size = atoi(sizeStr.c_str());
	}
	int childNum = xmlNode->children.size();

	// TODO: implement double later
	osg::ref_ptr<osg::FloatArray> buffer(new osg::FloatArray);

	for (int j = 0; j < size; j++)
	{
		for (int i = 0; i < childNum; i++)
		{
			osgDB::XmlNode *childNode = xmlNode->children[i].get();
			std::string typeString = childNode->properties["type"];
	...
	// TODO: impl double later
	int blockSize = buffer->size() * sizeof(GLfloat);
	...
	{
		ubb->setDataVariance(Object::STATIC);
	}
	else if (dataVariance == "dynamic")
	{
		ubb->setDataVariance(Object::DYNAMIC);
	}

	int globalFlag = atoi(xmlNode->properties["global"].c_str());
	if (globalFlag > 0) getOrCreateStateSet()->setAttributeAndModes(ubb.get(), osg::StateAttribute::ON);

	if (asGlobal)
	{
		if (!setUniformBufferBinding(name, ubb.get(), buffer.get()))
			OSG_NOTICE << "EffectCompositor: <uniform_buffer> object name " << name << " already exists" << std::endl;
		return ubb.get();
	}
	else
		return ubb.release();
...
osg::Uniform* EffectCompositor::createUniformFromXML(osgDB::XmlNode* xmlNode, bool asGlobal)
{
	...
	std::string isArrayStr = xmlNode->properties["isArray"];
	int globalFlag = atoi(xmlNode->properties["global"].c_str());

	// TODO: find more elegant way to do this for struct array
	// currently, struct array size for each member must be 
	// set to equal size manually, which is error prone.
	if (isArrayStr == "1")
	{
		// get array num
		...
		{
			osgDB::XmlNode *xmlChild = xmlNode->children[i].get();
			if (xmlChild->name != "value") continue;

			int arrIndex = atoi(xmlChild->properties["index"].c_str()); // TODO: handle atoi undefined if invalid str
			if (arrIndex > numElement || arrIndex < 0)
			{
				OSG_WARN << "EffectCompositor: <uniform> " << name 
					<< " doesn't have a recognizable array index: " << xmlChild->getTrimmedContents() << std::endl;
			}
			else
			{
				OSG_WARN << "To be implemented, currently only float is working" << std::endl;
				float f;
				std::stringstream ss;
				ss << xmlChild->getTrimmedContents();
				ss >> f;
				uniform->setElement(arrIndex, f);
			}
		}
...
osg::Shader* EffectCompositor::createShaderFromXML(osgDB::XmlNode* xmlNode, bool asGlobal)
{
	...
	std::string filePath;
	std::string versionStr = "#version 120\n";
	...
	for (unsigned int i = 0; i < xmlNode->children.size(); ++i)
	{
		...
		{
			versionStr = "#version " + xmlChild->getTrimmedContents() + "\n";
		}
		else if (childName == "source")
		{
			if (xmlChild->children.size() > 0)
			{
				...
				{
					// shaderStr = composeShaderSourceStr(versionStr, includeSrc, shaderStr);
			...
			else
			{
				...
				// shader->setShaderSource(composeShaderSourceStr(versionStr, includeSrc, xmlChild->getTrimmedContents()));
			}
		}
	...
	// strip #version directive in the shader source, let <version> handle it
	while ((pos = code.find("#version", pos)) != std::string::npos)
	{
		// Find all "#version" and delete them
		std::string::size_type pos2 = code.find_first_of("\r\n", pos + 8);
		code.erase(pos, pos2 - pos);
	}

	code = versionStr + code;
...
bool EffectCompositor::loadFromXMLFile(const std::string& filename)
{
	osg::ref_ptr<osgDB::XmlNode> xmlRoot = osgDB::readXmlFile(filename);
	if (xmlRoot.valid())
	{
		...
		this->loadFromXML(xmlRoot.get(), templateMap, NULL);
...
bool EffectCompositor::loadFromXML(osgDB::XmlNode* xmlNode, XmlTemplateMap& templateMap, const osgDB::Options* options)
{
	...
	for (unsigned int i = 0; i < xmlNode->children.size(); ++i)
	{
		...
		if (childName == "technique")
		{
			...
			createUniformBufferFromXML(xmlChild, true);
		else if (childName == "shader")
