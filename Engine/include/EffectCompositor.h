...
public:
	...
	bool setUniformBufferBinding(const std::string &name, osg::UniformBufferBinding *ubb, osg::FloatArray *floatBuffer);
	osg::UniformBufferBinding *getUniformBufferBinding(const std::string &name);
	const osg::UniformBufferBinding *getUniformBufferBinding(const std::string &name) const;

	// TODO: impl double array later
	typedef std::map <std::string, osg::ref_ptr<osg::UniformBufferBinding>> UniformBufferMap;
	typedef std::vector<osg::ref_ptr<osg::FloatArray> > UniformFloatBuffer;
	const UniformBufferMap &getUniformBufferMap() const { return _uniformBufferMap; }
    ...
    */
    ...
    void setPreservedNearAndFarAndClampMat( unsigned int frame, double zn, double zf, const osg::Matrix &clampedProjMat );
    ...
    */
	...
	osg::UniformBufferBinding* createUniformBufferFromXML(osgDB::XmlNode *xmlNode, bool asGlobal);
    ...
    */
	...
	/** Load effect from XML file directly */
...
protected:
	...
	void cacheUniformFloatBuffer(osg::FloatArray *fbuffer);

    PassListMap _passLists;
    TextureMap _textureMap;
    UniformMap _uniformMap;
	UniformBufferMap _uniformBufferMap;
	UniformFloatBuffer _uniformFloatBuffer;
	...
	osg::Matrix _preservedClampedProjectionMatrix;
