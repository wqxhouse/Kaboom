...
vec4 SMAANeighborhoodBlendingPS(vec2 texcoord, vec4 offset)
{
    ...
    else
    {
        ...
        // Fetch the opposite color and lerp by hand:
        vec4 C = texture2DLod(sceneTex, texcoord, 0.0);
		//vec2 txcord = texcoord + sign(offset) * SMAA_PIXEL_SIZE;
		vec2 txcord;
		txcord.x = texcoord.x + sign(offset.x) * SMAA_PIXEL_SIZE.x;
		txcord.y = texcoord.y + sign(offset.y) * SMAA_PIXEL_SIZE.y;
        // texcoord += sign(offset) * SMAA_PIXEL_SIZE;
        vec4 Cop = texture2DLod(sceneTex, txcord, 0.0);
